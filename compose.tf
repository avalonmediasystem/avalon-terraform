...
resource "aws_instance" "compose" {
  ...
  user_data = base64encode(templatefile("scripts/compose-init.sh", {
    ...
    fcrepo_ocfl_bucket_access_key = length(var.fcrepo_ocfl_bucket_username) > 0 ? var.fcrepo_ocfl_bucket_access_key : values(aws_iam_access_key.fcrepo_ocfl_created_access)[0].id 
    fcrepo_ocfl_bucket_secret_key = length(var.fcrepo_ocfl_bucket_username) > 0 ? var.fcrepo_ocfl_bucket_secret_key : values(aws_iam_access_key.fcrepo_ocfl_created_access)[0].secret 
    fcrepo_ocfl_bucket_id = aws_s3_bucket.fcrepo_ocfl_bucket.id
